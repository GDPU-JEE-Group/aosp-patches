...
set_restriction() {
    ...
    $IPTABLES -A droidwall-eth0 -m owner --uid-owner $uid -j droidwall-reject || exit
}
...
unset_restriction() {
    ...
    # 删除该应用的流量限制规则
    $IPTABLES -D droidwall-3g -m owner --uid-owner $uid -j droidwall-reject || exit
    $IPTABLES -D droidwall-wifi -m owner --uid-owner $uid -j droidwall-reject || exit
    $IPTABLES -D droidwall-eth0 -m owner --uid-owner $uid -j droidwall-reject || exit
}
