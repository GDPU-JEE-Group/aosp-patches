...
if /data/user/0/com.aidian.flowhelper/app_bin/iptables_armv5 --version >/dev/null 2>/dev/null ; then
        ...
        pro(){
          run_cmd $IPTABLES --version || exit 1

          # Flush existing rules
          run_cmd $IPTABLES -F || exit 2

          # Create the droidwall chains if necessary
          run_cmd $IPTABLES -L droidwall >/dev/null 2>/dev/null || $IPTABLES --new droidwall || exit 3
          run_cmd $IPTABLES -L droidwall-reject >/dev/null 2>/dev/null || $IPTABLES --new droidwall-reject || exit 4

          # Add droidwall chain to OUTPUT chain if necessary
          run_cmd $IPTABLES -L OUTPUT | $GREP -q droidwall || $IPTABLES -A OUTPUT -j droidwall || exit 5
          run_cmd $IPTABLES -L OUTPUT | $GREP -q droidwall-reject || $IPTABLES -A OUTPUT -j droidwall-reject || exit 6
        }
...
set_restriction() {
    ...
    uid=$2
        ...
        # Filtering rules
        # Reject all traffic for the specified UID (10094)
        $IPTABLES -A droidwall -m owner --uid-owner $UID -j REJECT || exit 7

        # Reject both incoming and outgoing traffic for UID 10094
        $IPTABLES -A INPUT -m owner --uid-owner $UID -j REJECT || exit 8
        $IPTABLES -A OUTPUT -m owner --uid-owner $UID -j REJECT || exit 9

        # If necessary, block DNS for the specific UID (UDP 53)
        $IPTABLES -A OUTPUT -p udp --dport 53 -m owner --uid-owner $UID -j DROP || exit 10
